...
namespace Zeroit.Framework.Utilities.SevenZip.Sdk.Compression.Lzma
{
  ...
  {
    private readonly uint[] _alignPrices = new uint[Convert.ToUInt32(new IntPtr(16))];
    private readonly uint[] _distancesPrices = new uint[Convert.ToUInt32(new IntPtr(512))];
    private readonly BitEncoder[] _isMatch = new BitEncoder[Convert.ToInt32(new IntPtr(192))];
    private readonly BitEncoder[] _isRep = new BitEncoder[Convert.ToInt32(new IntPtr(12))];
    private readonly BitEncoder[] _isRep0Long = new BitEncoder[Convert.ToInt32(new IntPtr(192))];
    private readonly BitEncoder[] _isRepG0 = new BitEncoder[Convert.ToUInt32(new IntPtr(12))];
    private readonly BitEncoder[] _isRepG1 = new BitEncoder[Convert.ToInt32(new IntPtr(12))];
    private readonly BitEncoder[] _isRepG2 = new BitEncoder[Convert.ToInt32(new IntPtr(12))];
    ...
    private readonly uint[] _matchDistances = new uint[Convert.ToInt32(new IntPtr(548))];
    private readonly Encoder.Optimal[] _optimum = new Encoder.Optimal[Convert.ToInt32(new IntPtr(4096))];
    private readonly BitEncoder[] _posEncoders = new BitEncoder[Convert.ToInt32(new IntPtr(114))];
    private readonly BitTreeEncoder[] _posSlotEncoder = new BitTreeEncoder[Convert.ToInt32(new IntPtr(4))];
    ...
    private readonly uint[] _repDistances = new uint[Convert.ToUInt32(new IntPtr(4))];
    ...
    private readonly uint[] repLens = new uint[Convert.ToUInt32(new IntPtr(4))];
    private readonly uint[] reps = new uint[Convert.ToUInt32(new IntPtr(4))];
    private readonly uint[] tempPrices = new uint[Convert.ToUInt32(new IntPtr(128))];
    ...
    private static readonly string[] kMatchFinderIDs = new string[2]{ "BT2", "BT4" };
      ...
      for (uint index = 0; (long) index < (long) properties.Length; ++index)
      {
        ...
        switch (propIDs[ index])
        {
          ...
          case CoderPropId.MatchFinder:
            ...
            int matchFinder = Encoder.FindMatchFinder(((string) property).ToUpper(CultureInfo.CurrentCulture));
            if (matchFinder < 0)
              throw new InvalidParamException();
...
label_45:
            ...
            else
            {
              ...
              else
              {
                ...
                for (int index = 3; index >= 1; --index)
                  this._repDistances[index] = this._repDistances[index - 1];
    ...
    {
      private readonly BitTreeEncoder[] _lowCoder = new BitTreeEncoder[Convert.ToInt32(new IntPtr(16))];
      private readonly BitTreeEncoder[] _midCoder = new BitTreeEncoder[Convert.ToInt32(new IntPtr(16))];
    ...
    {
      private readonly uint[] _counters = new uint[Convert.ToUInt32(new IntPtr(16))];
      private readonly uint[] _prices = new uint[Convert.ToUInt32(new IntPtr(4352))];
    ...
    private class LiteralEncoder
    {
      ...
      internal Encoder.LiteralEncoder.Encoder2 GetSubCoder(uint pos, byte prevByte)
      {
